...
def load_index(passage_embeddings,  faiss_gpu_index):
    ...
    if faiss_gpu_index:
        if len(faiss_gpu_index) == 1:
            ...
            co.useFloat16 = False
            index = faiss.index_cpu_to_gpu(res, faiss_gpu_index[0], index, co)
        else:
...
def main():
    ...
    config = BertConfig.from_pretrained(args.init_path)
    model = BertDot.from_pretrained(args.init_path, config=config, use_mean=False)
