...
def prediction(model, data_collator, args, test_dataset, embedding_memmap, ids_memmap):
...
def query_inference(model, args, embedding_size):
    ...
    try:
        prediction(model, query_collator, args,
                query_dataset, query_memmap, queryids_memmap)
    except:
...
def doc_inference(model, args, embedding_size):
    ...
    try:
        prediction(model, doc_collator, args,
            doc_dataset, doc_memmap, docid_memmap)
    except:
...
def main():
    ...
    # rgs.model_path = f"sentence-transformers/msmarco-bert-base-dot-v5"
    args.model_path = f"./data/{args.data_type}_sbert/star_train/models"
    args.output_dir = f"./data/{args.data_type}_sbert/evaluate/star"
